...
class AdminController extends AbstractController
{
     ...
     */
    public function ajout_article(Request $request, EntityManagerInterface $manager, PanierService $panierService)
    {
    ...
    public function ajout_categorie(Request $request, EntityManagerInterface $manager, Categorie $categorie=null){
        ...
        if(!$categorie): //si il n'y a pas d'objet categorie, on instancie un nouvel objet categorie, alors on est en ajout
     ...
     */
    public function liste_categories(CategorieRepository $categorieRepository)
    {

        $categories=$categorieRepository->findAll(); //on appelle ici le repository d'article qui nous permet d'effectuer les requêtes de SELECT. Ici SELECT * correspondant à findAll()
     ...
     */
    public function commande(PanierService $panierService, EntityManagerInterface $manager, $param=null, SessionInterface $session){
     ...
     */
    public function send_mail(Request $request, PanierService $panierService){
    ...
    */
    public function ajout_promo(Request $request, EntityManagerInterface $manager, UserRepository $userRepository){
        ...
        if($form->isSubmitted() && $form->isValid()): 
            ...
            $manager->flush();
            $this->addFlash('success', 'La promo a bien été ajoutée');

            return $this->redirectToRoute('liste_promo');
        endif;
    ...
    /**
     * @Route ("/liste_promo", name="liste_promo")
     */
    public function liste_promo(PromoRepository $promoRepository){

        $promos= $promoRepository->findAll();

        return $this->render('admin/liste_promo.html.twig', [
            'promos'=>$promos
        ]);
     ...
     */
    public function verif_promo(Request $request,  PromoRepository $promoRepository, PanierService $panierService, SessionInterface $session)
    {
        $panier=$panierService->getFullPanier();
        ...
        return $this->render('front/panier.html.twig',[
            ...
            $this->addFlash('alert', 'Votre code promo nest pas valide');
            return $this->redirectToRoute('cart', [
                'panier'=>$panier
            ]);
        endif;
